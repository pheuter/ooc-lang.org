...
<code>bash -c "`curl -L http://ooc.heroku.com/install.sh`"</code>

with wget:

<code>bash -c "`wget -O - http://ooc.heroku.com/install.sh`"</code> */


/* You can always compile it yourself: 
<ol>
  <li><code>git clone git@github.com:nddrylliog/rock.git</li>
  <li><code>cd rock</code></li>
  <li>Download <a href="http://github.com/downloads/nddrylliog/rock/rock-0.9.2-prealpha3-bootstrap-only.tar.gz2">boostrap source</a> and extract into the rock directory. A folder called <i>build</i> should appear.</li>
