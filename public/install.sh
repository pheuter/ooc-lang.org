...
function do_install {
    ...
    if [ -x "`which curl`" ]; then
        curl -L -\# "http://github.com/downloads/nddrylliog/rock/rock-0.9.2-prealpha3-bootstrap-only.tar.gz2" | tar -xf -
    elif [ -x "`which wget`" ]; then
        wget --progress=bar "http://github.com/downloads/nddrylliog/rock/rock-0.9.2-prealpha3-bootstrap-only.tar.gz2" -O - | tar -xf -
    fi
