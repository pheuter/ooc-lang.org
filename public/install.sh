...
function do_install {
    ...
    if [ -x "`which curl`" ]; then
        curl -L -\# "http://github.com/downloads/nddrylliog/rock/rock-0.9.2-prealpha5-bootstrap-only.tar.bz2" | tar -xjf -
    elif [ -x "`which wget`" ]; then
        wget --progress=bar "http://github.com/downloads/nddrylliog/rock/rock-0.9.2-prealpha5-bootstrap-only.tar.bz2" -O - | tar -xjf -
    fi
